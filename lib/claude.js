...
// Loosely extract a JSON value from a model's text output (schema-by-parse pattern
// from news-briefing/excerpt-extractor.js): strip a ```json fence, then parse the
// first {...} or [...] block. Returns the parsed value, or null on failure so the
// caller can gracefully fall back. The cluster/diff stages use no structured-output
// mode — they validate by parse.
function extractJson(text) {
  if (!text || typeof text !== 'string') return null;
  const stripped = text.replace(/```(?:json)?/gi, '');
  const m = stripped.match(/[[{][\s\S]*[\]}]/); // greedy: outermost block
  if (!m) return null;
  try { return JSON.parse(m[0]); } catch { return null; }
}

module.exports = { callClaudeOnce, callClaude, callHaiku, isRetryable, extractJson, DEFAULT_MODEL, HAIKU_MODEL };
